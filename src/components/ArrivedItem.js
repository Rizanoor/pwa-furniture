function numberFormat(updatedAt) {
    const currency = Intl.NumberFormat("id-ID", {
        style: "currency",
        currency: "IDR"
    });
    return currency.format(updatedAt);
}
...
function ArrivedItem({ item }) {
    return (
        <div className="px-4 relative card group">
            ...
            <h5 className="text-lg font-semibold mt-4">{item.slug}</h5>
            <span className="">{ numberFormat(item.updatedAt)}</span>
